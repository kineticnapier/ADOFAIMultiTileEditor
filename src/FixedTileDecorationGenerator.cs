...
namespace KineticNapier.ADOFAIMultiTileEditor
{
    internal static class FixedTileDecorationGenerator
    {
            ...
            IList<TrackSlot> tracks)
        {
            ...
            // PACL2AutoGenerator historically placed every moving planet one tile left
            // of its pivot. That makes a perfectly valid orbit but gives the whole planet
            // path a 180-degree phase error whenever the source chart starts in another
            // direction. Align only pairs that still look like MTE's deterministic auto
            // layout; manually positioned pairs remain untouched.
            ...
            return new TileDecorationResult
            {
                ...
                Diagnostic = "Generated " + created + " geometry-correct source Floor decoration(s); replaced " + replaced
                    + " previous MTE tile decoration(s); aligned " + alignedPlanetPairs
                    + " auto-created planet pair(s) to the source first-step phase"
        ...
        private static VisualTrack CaptureCurrentTrack(scnEditor editor, TrackSlot slot, int trackIndex)
        {
            ...
            Vector2 firstPosition = ToLevelPosition(floors[1], tileSize);
            Vector2 initialStep = firstPosition - startPosition;
            if (initialStep.sqrMagnitude > 1.0e-8f)
                visual.InitialStepOffset = initialStep;
        ...
        private static int AlignAutoGeneratedPlanetPairs(
            IList decorations,
            IList<TrackSlot> tracks,
            IList<VisualTrack> visuals)
        {
            int aligned = 0;
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                // Only touch the deterministic auto-layout. A complete manually-authored
                // pair can use arbitrary position/relativeTo and must remain untouched.
                ...
                if (!Approximately(movingPosition, legacyMoving)
                    && !Approximately(movingPosition, desiredMoving))
                    continue;
        ...
        private static LevelEvent FindPlanetAddObject(IList decorations, string requestedTag)
        {
            ...
            for (int i = 0; i < decorations.Count; i++)
            {
                ...
                // Duplicates are rejected by PACL2AutoGenerator before this point, but
                // keep this helper conservative if it is ever called independently.
